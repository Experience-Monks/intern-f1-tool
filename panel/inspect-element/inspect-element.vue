<div class="inspect-element main-pane">
  <h2>
    {{id}}
  </h2>
  <ul class="states">
    <li v-repeat="state in states">
      <button v-on="click: gotoState($key)">{{$key}}</button>
      <ul class="properties">
        <li v-repeat="property in state">
          <h4>{{$key}}</h4>
          <div v-if="$key === 'alpha'">
            <input type="range" min=0 max=1 v-model="property" step=0.01 number>
